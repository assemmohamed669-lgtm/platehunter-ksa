...
/**
 * هل الصفّين دول **نفس صف الداتا**؟ — مقارنة بالقيم مش بالمرجع ولا باللوحة.
 *
 * 🔴 ليه ده لازم: «موقعها» بتحلّ موضع الصف من رقم متخزّن وقت الفرز. لو الرقم
 * ده بقى قديم (الملف اتغيّر · ورقة اتشالت · فلتر اختلف) الكود كان بيعرض الصف
 * اللي الرقم وقع عليه **بلا أي تحقّق** — يعني ممكن يوري المندوب **عربية
 * تانية خالص** وهو فاكر إنها بتاعته ويروح لها.
 *
 * والتحقّق باللوحة وحده مش كافي: لوحة واحدة ممكن تكون مسجّلة **كذا مرة** في
 * مواضع مختلفة (ده بالظبط بلاغ المالك ٢٤ سبتمبر ٢٠٢٦)، فمقارنة اللوحة بتعدّي
 * على أي نسخة منهم وتقول «تمام» وهي على الصف الغلط.
 *
 * المقارنة بتعدّي على القيم المشتركة بين الصفّين: لو **كلها** متطابقة بعد
 * التشذيب ⇒ نفس الصف. الصف الفاضي أو اللي مافيش بينه وبين التاني أي عمود
 * مشترك بيرجّع `false` — «مش متأكد» = مش نفس الصف.
 */
export function sameDataRow(
  ...
  for (const k of keys) {
    ...
    if (!va && !vb) continue;          // العمودان فاضيين — مابيفرّقوش
    if (va !== vb) return false;
    compared++;
  }
  return compared > 0;                 // لازم قيمة واحدة على الأقل اتقارنت فعلاً
}
