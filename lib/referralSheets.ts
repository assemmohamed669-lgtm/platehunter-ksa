...
/**
 * الورقات **الظاهرة** بس — قرار صارم من المالك (٢٠٢٦-٠٩-٢١): البرنامج مايفرزش
 * على ورقة مخفية أبداً، الصفحة الأساسية بس.
 *
 * السبب: محافظ بتنزل وفيها ورقة عمل مخفية (نسخة قديمة/أسطول كامل) والفرز كان
 * بياخدها معاها. «محفظة التيسير المؤتمن الاول» مثال: المخفية `Sheet1` فيها
 * ١٣٩١ صف وهي **أول ورقة** في الملف، والمطلوبين الحقيقيين ٣٨٦ في الورقة
 * الظاهرة — فأي قارئ بياخد «أول ورقة» بياخد الغلط.
 *
 * **صمام أمان:** لو كل الورقات مخفية بنرجّعها كلها — نرفض الفلترة أهون من
 * إننا نرفض الملف كله.
 */
export function visibleSheets<T extends { hidden?: boolean }>(sheets: T[]): T[] {
  const shown = sheets.filter((s) => s.hidden !== true);
  return shown.length > 0 ? shown : sheets;
}
