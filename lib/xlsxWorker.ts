    ...
    if (!password && !forcedSheet) {
      try {
        ...
        if (withRows.length > 0) {
          ...
          //
          // 🔴 كان فيه استثناء لورقة «تشييك» (نفترض إنها الداتا والباقي ضجيج)
          // بيرمي باقي الورقات في صمت — وده بالظبط اللي ضيّع لوحة مطلوبة: ملف
          // تشييك الداتا الحقيقية فيه في ورقة «العربي» وورقة «تشييك» ملخّص بلا
          // لوحات. شيلنا الاستثناء: بندمج كل ورقة فيها لوحات فعلاً (شرط
          // العدّ > 0 بيتخطّى ورقات التعليمات/الملخّص)، فمافيش لوحة تضيع.
          const extras = withRows.filter(
