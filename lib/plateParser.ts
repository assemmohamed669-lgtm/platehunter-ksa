...
const SPOKEN_NUMBERS: [string, string][] = ([
  ...
  // «ثلاثه» بالهاء كانت **الوحيدة الناقصة** في عيلة الهاء (اربعه/خمسه/سته/سبعه/
  // تسعه كلهم موجودين). مرصودة في جلسة ميدانية ٢٠٢٦/٠٨/٠٢: «اثنين ثلاثه تسعتسع»
  // — الكلمة فضلت خردة فقطعت صف الأرقام لنُصّين وطلّعت لوحتين بدل واحدة.
  ["ثلاثة",  "3"], ["تلاتة",  "3"], ["تلاته",  "3"], ["ثلاثه",  "3"], ["ثلاث",   "3"],
  ["تلات",   "3"], ["تلته",   "3"],
  ...
  // ── إملاءات Deepgram الحقيقية لنفس الصيغ (جلسة ميدانية ٢٠٢٦/٠٨/٠٢) ────────
  //   القاموس فوق اتكتب بشكل واحد لكل صيغة، والميدان طلّع شكلين تانيين. تلات
  //   غلطات من ست في الجلسة كانت من النقص ده وحده — مش من الموديل ولا الـASR.
  //   ١. **التاء المربوطة**: الأنبوب بيحوّل `أ→ا` بس؛ `ة` عمرها ما بتتحوّل، فـ
  //      «اربعة وحايد» مابتطابقش «اربع وحايد». (مرصودة: «أربعة وحايد» ⇒ طصص0004.)
  ["اربعة اصفار",   "0000"], ["اربعة تصفار",   "0000"],
  ["اربعة وحايد",   "1111"],
  ["اربعة اتنينات", "2222"],
  ["اربعة تلاتات",  "3333"], ["اربعة تلتات",   "3333"], ["اربعة ثلاثات", "3333"],
  ["اربعة اربعات",  "4444"],
  ["اربعة خمسات",   "5555"],
  ["اربعة سبعات",   "7777"],
  ["اربعة تمنيات",  "8888"], ["اربعة تمانيات", "8888"], ["اربعة ثمانيات", "8888"],
  ["اربعة تسعات",   "9999"],
  //   ٢. **ملزوقة بلا مسافة**: Deepgram بيلزق العدد بالمعدود في الكلام السريع.
  //      (مرصودة: «أربعسبعات» ⇒ مافيش لوحة خالص.) كلها **مش كلمات عربية** فبلا
  //      أي تصادم — ومنها «تلاتصفار» اللي كانت متضافة قبل كده لوحدها.
  ["اربعاصفار",   "0000"], ["اربعتصفار",   "0000"],
  ["اربعوحايد",   "1111"],
  ["اربعاتنينات", "2222"],
  ["اربعتلاتات",  "3333"], ["اربعتلتات",   "3333"], ["اربعثلاثات", "3333"],
  ["اربعاربعات",  "4444"],
  ["اربعخمسات",   "5555"],
  ["اربعسبعات",   "7777"],
  ["اربعتمنيات",  "8888"], ["اربعتمانيات", "8888"], ["اربعثمانيات", "8888"],
  ["اربعتسعات",   "9999"],
  ["تلاتاصفار",   "000"], ["تلاتتصفار",   "000"], ["تلاتوحايد",   "111"],
  ["تلاتاتنينات", "222"], ["تلاتتلاتات",  "333"], ["تلاتاربعات",  "444"],
  ["تلاتخمسات",   "555"], ["تلاتستات",    "666"], ["تلاتسبعات",   "777"],
  ["تلاتتمنيات",  "888"], ["تلاتتسعات",   "999"],
  //   ٣. **رقم مكرر ملزوق**: «تسعة تسعة» بتطلع «تسعتسع» = خانتين مش واحدة.
  //      (مرصودة: «تسعتسع» ⇒ اوب0002 بدل اوب2399.) برضه مش كلمات عربية.
  //      المرصود هو «تسعتسع»؛ الباقي نفس النمط وبلا تصادم فاتضاف معاه.
  ["صفرصفر", "00"], ["واحدواحد", "11"], ["اتنيناتنين", "22"], ["تلاتتلات", "33"],
  ["اربعاربع", "44"], ["خمسخمس", "55"], ["ستست", "66"], ["سبعسبع", "77"],
  ["تمنتمن", "88"], ["تسعتسع", "99"],
  ...
  // أو من الكود الأصلي).
  // ── «ثلاثة» بالتاء المربوطة — العيلة كانت ناقصة (تلات/ثلاث/تلاته/تالت
  //    موجودين، و«ثلاثة» لأ). مرصودة في جلسة ميدانية ٢٠٢٦/٠٨/٠٢:
  //    «دال حا ثلاثة تصفار تسعة» ⇒ الكلمة فضلت خردة فقطعت صف الأرقام
  //    وطلّعت **لوحة وهمية** (صار0009) جنب اللوحة الناقصة.
  ["ثلاثة اصفار", "000"],
  ["ثلاثة تصفار", "000"],
  ["ثلاثة وحايد", "111"],
  ["ثلاثة اتنينات", "222"],
...
export interface MatchResult {
  ...
  refPlateNorm?: string;
  // موضع صف الداتا في ترتيب collectDataSources (الملفات ورا بعض) — بيتخزّن وقت
  // الفرز فيصمد بعد إعادة الفتح (لميزة «موقعها» — جيران نفس الشارع).
  dataIdx?: number;
  /**
   * رقم ملف الداتا اللي الصف جه منه (٠ = الملف الأول). بيتخزّن وقت الفرز عشان
