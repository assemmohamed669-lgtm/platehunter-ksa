...
export interface FieldCheckEntry {
  ...
  /**
   * معرّف الصف اللي اتصدّر منه (صف الصوت أو الكاميرا). بيربط السجل المحفوظ
   * بالصف اللي لسه ظاهر في القائمة — فلو المندوب عدّل النوع أو اللوحة **بعد**
   * التصدير، التعديل يوصل للسجل المحفوظ كمان مش يفضل في الذاكرة بس.
   */
  srcId?: string;
}
