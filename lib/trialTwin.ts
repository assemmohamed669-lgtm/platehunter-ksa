...
/**
 * ══════════════════════════════════════════════════════════════════════
 *  نفس النطقة **بزمن الصوت** — للقرايات المتأخّرة بعد وقعة الشبكة
 * ══════════════════════════════════════════════════════════════════════
 *
 * إعادة إرسال النوافذ الفايتة (`lib/voicexReplay.ts`) بتجيب قرايات **متأخّرة
 * بـ٢٠+ ثانية** بس بزمن صوتها الأصلي. ولو اللوحة ظهرت خلاص من نافذة نجحت وقت
 * الوقعة، `isExactRepeatNearby` بتبص على **أول ٣ صفوف بالمكان** — واللوحة دي
 * بقت تحت ⇒ كانت هتتحسب «إعادة مقصودة» ⇒ **صف مكرّر**.
 *
 * ⇒ نفس اللوحة بالحرف **وزمن صوتها قريب** = نفس النطقة، مهما كان مكانها.
 *
 * ⚠️ **٥ ثواني**: قرايات النطقة الواحدة بتيجي من ٢-٤ نوافذ متتالية (خطوة
 *    ١.٥ث) فمراكزها بتبعد عن بعض لحد ~٤.٥ث. ونطقتين منفصلتين لنفس اللوحة
 *    مايتداخلوش في الصوت — والإعادة المقصودة اللي المالك قالها (`دمس5284`)
 *    كان بينها ٤ لوحات ⇒ ١٠+ ثواني.
 */
export const SAME_UTTERANCE_MS = 5000;

export function sameUtterance(a: TwinRow, b: TwinRow): boolean {
  const pa = String(a?.plate ?? ""), pb = String(b?.plate ?? "");
  if (!pa || pa !== pb) return false;
  return Math.abs((a.atMs ?? 0) - (b.atMs ?? 0)) <= SAME_UTTERANCE_MS;
}
