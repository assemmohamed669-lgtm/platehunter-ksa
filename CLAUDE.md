...
### ملفات داتا متعددة (زر "+ إضافة ملف داتا")

- نفس فكرة الإحالات المتعددة بس للداتا: زر "**+ إضافة ملف داتا**" في مربع الداتا يضيف صناديق (داتا ٢، ٣...). كل ملفات الداتا **بتتدمج** في كل مسارات الفرز (جديد/كلي/**مطلوب**/لصق) عبر `collectDataSources()` (في `sorting/page.tsx`) — كل مصدر بعمود لوحته (`detectArabicPlateColumn ?? detectPlateColumn`).
- تتخزّن في IDB slots متتابعة (`data-2`, `data-3`...) فتفضل بعد إعادة الفتح. صفحة «المطلوب» (`wanted/page.tsx`) بتقرا كل slots الداتا وتطابق عليها كلها.
- أعمدة النتيجة بتتحل من الملف الأساسي (الملفات الإضافية مفترض نفس الأعمدة).
...
```ts
bankPlateToArabic("NKD 5678") // → "نكد5678"   (حروف ثم أرقام = ترتيب عربي)
bankPlateToArabic("7709 ABS") // → "سبا7709"   (أرقام ثم حروف = تخطيط لاتيني)
// خريطة: N→ن, K→ك, D→د, H→هـ, U→و, V→ي, A→ا, B→ب ...
```

**فيه تخطيطان للمحافظ الإنجليزية، والفرق بينهم مش شكلي:**

| التخطيط | مثال | المعالجة |
|---------|------|----------|
| **حروف ثم أرقام** | `NKD 5678` | ترتيب عربي — الحروف تتحوّل كما هي |
| **أرقام ثم حروف** | `7709 ABS` | تخطيط لاتيني — **الحروف تتعكس** وتترتّب حروف‑ثم‑أرقام |

السبب: على اللوحة السعودية العربي يُقرأ يمين‑لشمال واللاتيني شمال‑ليمين، فالسطرين
**مرآة لبعض**. محفظة منقولة من السطر اللاتيني بتنزل `7709 ABS` وهي فعلياً
`س ب ا 7709`. قبل الإصلاح البرنامج كان بيقراها حرف بحرف فيطلّع `7709ابس` —
الحروف مقلوبة والأرقام في الأول — فمافيش لوحة واحدة كانت بتطابق.

المُشغِّل هو **التخطيط نفسه**، مش اسم البنك: أرقام قبل الحروف = لاتيني.
متحقّق منه على ملف إحالة حقيقي — **55 من 55** صح بالعكس، **0 من 55** بدون عكس.
...
### قارئان للداتا — عادي vs دفعات (ومين بيقرا إيه)

| المسار | القارئ | بيفهم |
|--------|--------|-------|
| أي مربع رفع (إحالة/تشييك/داتا إضافية) وملف داتا **< ١٢ ميجا** | `parseExcelFile` (SheetJS) | كل الصيغ: xlsx / xlsb / xls / ods / csv |
| ملف الداتا الأساسي **> ١٢ ميجا** (`LARGE_DATA_THRESHOLD_BYTES`) | `streamXlsxToBatches` (دفعات، ذاكرة قليلة) | **xlsx / xlsm بس** |

- قارئ الدفعات بيفكّ الـzip ويدوّر على ورقة XML جوّه `xl/worksheets/`. متسامح مع:
  اسم بدون رقم (`sheet.xml`)، حروف كبيرة (`Sheet1.xml`)، بادئات namespace في العناصر
  والخصائص (`x:row`، `rel:id`) — كلها كانت بتفشل قبل كده.
- لو الملف **مش xlsx من جوّه** (xlsb/ods/أرشيف) بيرمي `NotXlsxWorksheetError` برسالة
  تسمّي الصيغة الحقيقية، و`importLargeDataFile` بيرجع تلقائياً للقارئ العادي فالمندوب
  مايتوقفش. نفس الاحتياطي بيشتغل لو القراءة رجّعت **صفر صفوف** (مثلاً أول ورقة فاضية).
- ⚠️ ملاحظة: قارئ الدفعات بياخد **أول ورقة** في الملف دايماً (مافيش أزرار تبديل ورقة
  للملفات الكبيرة). لو أول ورقة فيها صفوف قليلة غلط، الفرز هيمشي عليها بدون تنبيه.
