...
describe("resolveMergedResultColumns — دمج مصادر متعددة (إحالة إضافية)", () => {
  it("أعمدة الإحالة الإضافية تظهر لو مش موجودة في الداتا/الأساسية", () => {
    ...
    // العنوان من الداتا
    expect(byKey["address"]?.sources[0].source).toBe("data");
    // اللون والسنة من الإحالة الإضافية — كانوا بيضيعوا قبل الإصلاح
    expect(byKey["color"]?.sources[0].sourceCol).toBe("اللون");
    expect(byKey["color"]?.sources[0].source).toBe("referral");
    expect(byKey["year"]?.sources[0].sourceCol).toBe("سنة الصنع");
    expect(byKey["year"]?.sources[0].source).toBe("referral");
  });

  it("الأولوية للمصدر الأول: الداتا أول مصدر لنفس الهدف", () => {
    const data = {
      kind: "data" as const,
    ...
    expect(color?.sources[0].sourceCol).toBe("اللون"); // الداتا أول مصدر
    expect(color?.sources[0].source).toBe("data");
    // الإحالة موجودة كمصدر احتياطي
    expect(color?.sources.some((s) => s.source === "referral" && s.sourceCol === "لون المركبة")).toBe(true);
  });
});

describe("resultCellValue — قراءة أول قيمة غير فاضية (fallback)", () => {
  const col = {
    sources: [
      { source: "data" as const, sourceCol: "نوع السيارة" },
      { source: "referral" as const, sourceCol: "Type of car" },
    ],
  };
  it("قيمة الداتا لو موجودة تُستخدم", () => {
    expect(resultCellValue(col, { "نوع السيارة": "ملاكي" }, { "Type of car": "COROLLA" })).toBe("ملاكي");
  });
  it("لو الداتا فاضية نرجع لقيمة الإحالة (المشكلة اللي بلّغ عنها المستخدم)", () => {
    expect(resultCellValue(col, { "نوع السيارة": "" }, { "Type of car": "COROLLA" })).toBe("COROLLA");
    expect(resultCellValue(col, {}, { "Type of car": "COROLLA" })).toBe("COROLLA");
    expect(resultCellValue(col, undefined, { "Type of car": "COROLLA" })).toBe("COROLLA");
  });
  it("لو كل المصادر فاضية بترجّع فاضي", () => {
    expect(resultCellValue(col, { "نوع السيارة": "  " }, { "Type of car": "" })).toBe("");
  });
});
