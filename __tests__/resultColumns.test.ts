...
describe("resultColumns — ترتيب أعمدة النتيجة", () => {
  // بطلب المندوب: بعد رقم اللوحة على طول نوع السيارة، وبعده اسم الموقع
  // (العنوان ثم الحي)، وبعدهم باقي البيانات.
  it("اسم الموقع (العنوان/الحي) بييجي بعد نوع السيارة وقبل الماركة", () => {
    const headers = ["رقم اللوحة", "الماركة", "النوع", "العنوان", "الحي", "GPS", "اللون", "سنة الصنع", "تاريخ التسجيل"];
    const rows = [{
      "رقم اللوحة": "دحر1234", "الماركة": "تويوتا", "النوع": "صالون",
      "العنوان": "شارع الملك فهد", "الحي": "العليا", "GPS": "x",
      "اللون": "أبيض", "سنة الصنع": "2020", "تاريخ التسجيل": "15/05/2024",
    }];
    const res = resolveResultColumns(headers, rows, "رقم اللوحة");
    expect(res.map((c) => c.key)).toEqual([
      "type", "address", "district", "brand", "gps", "color", "year", "date",
    ]);
  });

  it("لو مفيش عنوان، الحي لوحده بييجي بعد النوع وقبل الماركة", () => {
    const headers = ["رقم اللوحة", "الماركة", "النوع", "الحي"];
    const rows = [{ "رقم اللوحة": "دحر1234", "الماركة": "تويوتا", "النوع": "صالون", "الحي": "العليا" }];
    const res = resolveResultColumns(headers, rows, "رقم اللوحة");
    expect(res.map((c) => c.key)).toEqual(["type", "district", "brand"]);
  });
});

describe("نتيجة فرز السجلات — بيانات السيارة بتيجي من المحفظة", () => {
  // الحالة اللي كانت بتحصل: شيت السجلات فيه لوحة/تاريخ/GPS بس، والمحفظة فيها
  // النوع والماركة واللون والسنة — فنتيجة السجلات كانت بتطلع لوحة وتاريخ
  // وموقع وخلاص. دلوقتي بندمج المصدرين فبيانات المحفظة بيبقى ليها أعمدة.
  it("أعمدة المحفظة تظهر حتى لو شيت السجلات مافيهوش غير اللوحة والتاريخ والموقع", () => {
    const cols = resolveMergedResultColumns([
      {
        kind: "data",
        headers: ["رقم اللوحة", "GPS", "التاريخ", "الحالة"],
        rows: [{ "رقم اللوحة": "دحر1234", "GPS": "https://maps.google.com/x", "التاريخ": "15/05/2024", "الحالة": "متشيكة بالكاميرا" }],
        plateCol: "رقم اللوحة",
      },
      {
        kind: "referral",
        headers: ["رقماللوحة", "طراز المركبة", "صانع المركبة", "لون المركبة الأساسي", "سنة الصنع"],
        rows: [
          { "رقماللوحة": "د ح ر 1234", "طراز المركبة": "كامري", "صانع المركبة": "تويوتا", "لون المركبة الأساسي": "أبيض", "سنة الصنع": "2020" },
          { "رقماللوحة": "د ح ر 5678", "طراز المركبة": "النترا", "صانع المركبة": "هيونداي", "لون المركبة الأساسي": "فضي", "سنة الصنع": "2019" },
          { "رقماللوحة": "د ح ر 9012", "طراز المركبة": "سوناتا", "صانع المركبة": "كيا", "لون المركبة الأساسي": "أسود", "سنة الصنع": "2021" },
        ],
        plateCol: "رقماللوحة",
      },
    ]);
    const by = Object.fromEntries(cols.map((c) => [c.key, c]));
    expect(by.type?.source).toBe("referral");      // النوع/الطراز من المحفظة
    expect(by.brand?.source).toBe("referral");     // الماركة من المحفظة
    expect(by.color?.source).toBe("referral");     // اللون من المحفظة
    expect(by.year?.source).toBe("referral");      // سنة الصنع من المحفظة
    expect(by.gps?.source).toBe("data");           // الموقع من السجل نفسه
    expect(by.date?.source).toBe("data");          // تاريخ التشييك من السجل
    // والترتيب: النوع الأول، وبعده الماركة (مفيش عنوان/حي هنا)
    expect(cols.map((c) => c.key)).toEqual(["type", "brand", "gps", "color", "year", "date"]);
  });
});

describe("resultColumns — نوع السيارة والموقع من الداتا قبل أي عمود محفظة", () => {
  // شيت داتا فيه «النوع» و«الحي»، ومحفظة فيها طراز + منطقة. المطلوب:
  // رقم اللوحة › نوع السيارة (الداتا) › الحي (الداتا) › باقي الخيارات.
  // و«الحي» بتاع المحفظة مايظهرش خالص (ده عنوان سجلّ البنك مش موقع التفريغ).
  const dataSrc = {
    kind: "data" as const,
    headers: ["رقم اللوحة", "النوع", "الحي", "GPS", "اللون"],
    rows: [{ "رقم اللوحة": "دحر1234", "النوع": "صالون", "الحي": "العليا", "GPS": "https://maps.google.com/x", "اللون": "أبيض" }],
    plateCol: "رقم اللوحة",
  };
  const refSrc = {
    kind: "referral" as const,
    headers: ["رقماللوحة", "طراز المركبة", "المنطقة"],
    rows: [
      { "رقماللوحة": "د ح ر 1234", "طراز المركبة": "كامري", "المنطقة": "الرياض" },
      { "رقماللوحة": "د ح ر 5678", "طراز المركبة": "النترا", "المنطقة": "جدة" },
      { "رقماللوحة": "د ح ر 9012", "طراز المركبة": "سوناتا", "المنطقة": "الدمام" },
    ],
    plateCol: "رقماللوحة",
  };

  it("«الحي (المحفظة)» مايظهرش خالص", () => {
    const cols = resolveMergedResultColumns([dataSrc, refSrc]);
    expect(cols.some((c) => c.key === "district" && c.source === "referral")).toBe(false);
    expect(cols.map((c) => c.label)).not.toContain("الحي (المحفظة)");
  });

  it("الترتيب: نوع السيارة (الداتا) › الحي (الداتا) › باقي الخيارات", () => {
    const cols = resolveMergedResultColumns([dataSrc, refSrc]);
    expect(cols.slice(0, 2).map((c) => c.label)).toEqual(["نوع السيارة", "الحي"]);
    expect(cols[0].source).toBe("data");
    expect(cols[1].source).toBe("data");
    // نوع المحفظة بيفضل موجود بس بعد الموقع
    expect(cols.map((c) => c.label)).toContain("نوع السيارة (المحفظة)");
    expect(cols.findIndex((c) => c.label === "نوع السيارة (المحفظة)")).toBeGreaterThan(1);
  });

  it("لو العنوان والحي الاتنين في الداتا → الاتنين يظهروا بعد النوع", () => {
    const both = {
      kind: "data" as const,
      headers: ["رقم اللوحة", "النوع", "العنوان", "الحي", "اللون"],
      rows: [{ "رقم اللوحة": "دحر1234", "النوع": "صالون", "العنوان": "شارع الملك فهد", "الحي": "العليا", "اللون": "أبيض" }],
      plateCol: "رقم اللوحة",
    };
...
describe("resultColumns — تحليل بالاسم", () => {
  it("أعمدة بأسماء معروفة تتحلّل بالترتيب الثابت", () => {
    ...
    const res = resolveResultColumns(headers, rows, "رقم اللوحة");
    // الترتيب الثابت: نوع، حي (اسم الموقع)، ماركة، GPS، لون، سنة، تاريخ. «الحي»
    // هدف مستقل (فيه عمود «الحي» مش «العنوان»)، فبيتحل كـ district مش address.
    expect(res.map((c) => c.key)).toEqual(["type", "district", "brand", "gps", "color", "year", "date"]);
...
describe("resolveMergedResultColumns — دمج مصادر متعددة (إحالة إضافية)", () => {
  ...
  it("محافظ إحالة متعددة بأعمدة مختلفة → عمود واحد لكل هدف يقرا من الكل", () => {
    // المشكلة: لما تفرز على كذا محفظة بأسماء أعمدة مختلفة (عربي/إنجليزي)، كل محفظة
    // كانت بتطلّع عمود منفصل، فاللوحة الجاية من محفظة تملى عمودها والباقي فاضي.
    // المطلوب: كل أعمدة الإحالة لنفس الهدف تتدمج في عمود واحد يقرا من أي محفظة.
    const data = {
      kind: "data" as const,
      headers: ["رقم اللوحة", "الحي"],
      rows: [{ "رقم اللوحة": "دحر1234", "الحي": "العليا" }],
    ...
    const ref1 = {
      kind: "referral" as const,   // محفظة عربية
      headers: ["رقم اللوحة", "صانع المركبة", "طراز المركبة"],
      rows: [
        { "رقم اللوحة": "دحر1234", "صانع المركبة": "تويوتا", "طراز المركبة": "كورولا" },
        { "رقم اللوحة": "دحر5", "صانع المركبة": "نيسان", "طراز المركبة": "صني" },
        { "رقم اللوحة": "دحر6", "صانع المركبة": "كيا", "طراز المركبة": "سيراتو" },
      ],
      plateCol: "رقم اللوحة",
    };
    const ref2 = {
      kind: "referral" as const,   // محفظة إنجليزية بأسماء مختلفة
      headers: ["رقم اللوحة", "Brand", "Car Type"],
      rows: [
        { "رقم اللوحة": "دحر7", "Brand": "دودج", "Car Type": "شارجر" },
        { "رقم اللوحة": "دحر8", "Brand": "تويوتا", "Car Type": "راف فور" },
        { "رقم اللوحة": "دحر9", "Brand": "متسوبيشي", "Car Type": "باجيرو" },
      ],
      plateCol: "رقم اللوحة",
    };
    const res = resolveMergedResultColumns([data, ref1, ref2]);
    const typeCols = res.filter((c) => c.key === "type");
    const brandCols = res.filter((c) => c.key === "brand");
    // عمود نوع واحد بس للإحالة (مش عمودين) بيضم أعمدة المحفظتين بالترتيب
    expect(typeCols).toHaveLength(1);
    expect(typeCols[0].source).toBe("referral");
    expect(typeCols[0].sourceCols).toEqual(["طراز المركبة", "Car Type"]);
    expect(typeCols[0].sourceCol).toBe("طراز المركبة"); // العمود الأساسي = أول مرشّح
    // نفس الشيء للماركة
    expect(brandCols).toHaveLength(1);
    expect(brandCols[0].sourceCols).toEqual(["صانع المركبة", "Brand"]);
  });
