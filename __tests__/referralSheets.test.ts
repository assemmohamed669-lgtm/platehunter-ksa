...
describe("defaultSelection — الاختيار التلقائي", () => {
  ...
  // محفظة «مجمع الجبر»: ورقتين بس (١٠٣ + ٧ لوحات) والاتنين مطلوبين. قاعدة
  // «الأسطول» كانت بتشيل الكبيرة فالفرز يشتغل على ٧ لوحات بس — والمندوب لازم
  // يعيد الفرز يدوي على التانية. نمط الأسطول الحقيقي بييجي مع **٣ ورقات أو
  // أكتر** (أسطول + مباعة + مسروقة…)، فورقتين بس مافيش فيهم دليل كفاية.
  it("ورقتين بس والكبيرة مهيمنة → الاتنين متعلّمين (مافيش دليل أسطول)", () => {
    const sheets = [sheet("76", 103), sheet("اليمنيه", 7)];
    const sel = defaultSelection(sheets);
    expect(sel.has("76")).toBe(true);
    expect(sel.has("اليمنيه")).toBe(true);
  });

  it("٣ ورقات والكبيرة مهيمنة → الكبيرة تفضل مستبعدة (سلوك الأسطول زي ما هو)", () => {
    const sheets = [sheet("vehicle_list", 1380), sheet("مباعة", 35), sheet("مسروقة", 28)];
    expect(defaultSelection(sheets).has("vehicle_list")).toBe(false);
  });
