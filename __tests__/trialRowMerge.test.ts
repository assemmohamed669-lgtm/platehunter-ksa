...
/**
 * ══════════════════════════════════════════════════════════════════════
 *  🔴 «طلعت مطلوبة وهي مش في الشيت، وبشهادة لوحة تانية»
 * ══════════════════════════════════════════════════════════════════════
 *  بلاغ المالك (٢٥ سبتمبر ٢٠٢٦): المندوب قال «رلم6146» (مش في شيته) —
 *  البرنامج صفّر وعلّمها مطلوبة، وتحتها بيانات وشهادة «رلم6113» (اللي في
 *  الشيت: تويوتا · راجحي نشط · هيكل MR2BE9B39R0088106). ولما قالها تاني
 *  ماصفّرش.
 *
 *  السبب: قراية غلط سمعتها «رلم6113» ⇒ الصف اتعلّم مطلوب. بعدين الإجماع
 *  صحّح اللوحة لـ«رلم6146» (مش مطلوبة ⇒ `match = null`) — والدمج أخد
 *  **مطابقة الصف القديم** (`fresh.match ?? twin.match`). فالصف بقى رقم لوحة
 *  ببيانات وشهادة لوحة تانية.
 *
 *  ⇒ اللوحة ومطابقتها **مع بعض دايماً**: اللوحة اتغيّرت ⇒ مطابقتها هي.
 */
describe("mergeTwinRow — اللوحة ومطابقتها مع بعض", () => {
  const wanted6113 = { "اللوحه": "رلم6113", "النوع": "تويوتا", "البنك": "راجحي نشط", "الشاص": "MR2BE9B39R0088106" };

  it("🔴 اللوحة اتصحّحت لواحدة مش مطلوبة ⇒ «مطلوبة» والبيانات بتتشال", () => {
    const twin = row({ plate: "رلم6113", match: wanted6113 });
    const fresh = row({ id: "B", plate: "رلم6146", match: null });
    const out = mergeTwinRow(fresh, twin);
    expect(out.plate).toBe("رلم6146");
    expect(out.match).toBeNull();
  });

  it("اللوحة اتصحّحت لواحدة مطلوبة تانية ⇒ مطابقة الجديدة", () => {
    const other = { "اللوحه": "رلم6146" };
    const out = mergeTwinRow(row({ id: "B", plate: "رلم6146", match: other }), row({ plate: "رلم6113", match: wanted6113 }));
    expect(out.match).toBe(other);
  });

  it("نفس اللوحة والقراءة الجديدة مالقتش مطابقة ⇒ القديمة تفضل (زي الأول)", () => {
    const out = mergeTwinRow(row({ id: "B", plate: "رلم6113", match: null }), row({ plate: "رلم6113", match: wanted6113 }));
    expect(out.match).toBe(wanted6113);
  });

  it("نفس اللوحة بفرق همزة/مسافة بس ⇒ بتتحسب نفس اللوحة", () => {
    const out = mergeTwinRow(row({ id: "B", plate: "ابح 1234", match: null }), row({ plate: "أبح1234", match: wanted6113 }));
    expect(out.match).toBe(wanted6113);
  });
});
