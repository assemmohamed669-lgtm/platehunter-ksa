...
/** نافذة واحدة سمعت اللوحات دي — زي المحرّك بالظبط: الدليل الأول وبعدين الإضافة. */
function feed(lc: LiveConsensus, fleet: FleetMemory | null, tMs: number, plates: string[], conf = 0.97, minLp?: number) {
  fleet?.note(plates);
  for (const p of plates) lc.add({ plate: p, tMs, conf, minLp } as PlateRead);
}
...
describe("🚚 الإجماع — أسطول متسلسل", () => {
  ...
  it("🔴 غلط سمع لعربية في الأسطول بيتلمّ فيها — مايطلعش عربية زيادة", () => {
    ...
    expect(out.find((c) => c.plate === "حبل1234")?.mult).toBe(3);
  });
