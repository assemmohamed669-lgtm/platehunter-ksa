...
describe("matchesPreferred — Arabic bank referral columns", () => {
  ...
  it("matches البنك (بطلب المستخدم — يظهر تلقائياً في النتيجة)", () => expect(matchesPreferred("البنك")).toBe(true));
  it("matches اسم البنك", () => expect(matchesPreferred("اسم البنك")).toBe(true));
