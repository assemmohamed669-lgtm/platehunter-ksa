...
describe("collectReferralEntries — دمج شيتات إحالة متعددة في قائمة واحدة", () => {
  it("يدمج شيتين ويحذف المكرر بالتطبيع (أول ظهور يكسب)", () => {
    const s1 = { rows: [{ p: "أ ب ح 1234" }, { p: "ن ك د 5678" }], plateCol: "p", isArabic: true };
    const s2 = { rows: [{ q: "ابح1234" }, { q: "ه ه ه 9999" }], plateCol: "q", isArabic: true };
    const out = collectReferralEntries([s1, s2]);
    // أبح1234 مكرر بين الشيتين → مرة واحدة؛ الباقي فريد
    expect(out.map((e) => e.norm)).toEqual(["ابح1234", "نكد5678", "ههه9999"]);
    // أول ظهور يكسب — الصف من s1 مش s2
    expect(out[0].row).toBe(s1.rows[0]);
  });

  it("يتخطى الخلايا الفارغة", () => {
    const s = { rows: [{ p: "" }, { p: "أ ب ح 1234" }, { p: "   " }], plateCol: "p", isArabic: true };
    expect(collectReferralEntries([s]).map((e) => e.norm)).toEqual(["ابح1234"]);
  });

  it("يطبّع لوحات البنك الإنجليزية (isArabic=false) للعربي", () => {
    const s = { rows: [{ p: "NKD 5678" }], plateCol: "p", isArabic: false };
    const out = collectReferralEntries([s]);
    expect(out[0].norm).toBe("نكد5678");
    expect(out[0].raw).toBe("NKD 5678");
    expect(out[0].isArabic).toBe(false);
  });
});

describe("plateNeedsReview — علامة اللوحة المكسورة الشكل", () => {
  it("لوحة سليمة (3 حروف + 4 أرقام) مش محتاجة مراجعة", () => {
    expect(plateNeedsReview("ابح1234")).toBe(false);
    expect(plateNeedsReview("قنص4455")).toBe(false);
  });
  it("أرقام بس (بدون حروف) → محتاجة مراجعة", () => {
    expect(plateNeedsReview("1234")).toBe(true);
  });
  it("حروف بس (بدون أرقام) → محتاجة مراجعة", () => {
    expect(plateNeedsReview("ابح")).toBe(true);
  });
  it("فاضية → محتاجة مراجعة", () => {
    expect(plateNeedsReview("")).toBe(true);
  });
  it("أكتر من 3 حروف (كلمات ما اتحولتش) → محتاجة مراجعة", () => {
    expect(plateNeedsReview("ابحد1234")).toBe(true);
  });
  it("أكتر من 4 أرقام → محتاجة مراجعة", () => {
    expect(plateNeedsReview("ابح12345")).toBe(true);
  });
  it("حرف مش من حروف اللوحات المعتمدة → محتاجة مراجعة", () => {
    expect(plateNeedsReview("ثبح1234")).toBe(true); // ث مش حرف لوحة سعودي
  });
...
describe("normalizeReferralPlate — تطبيع حسب نوع العمود", () => {
  it("عربي: تطبيع مباشر", () => {
    expect(normalizeReferralPlate("أ ب ح 1234", true)).toBe("ابح1234");
  });
  it("إنجليزي: تحويل ثم تطبيع", () => {
    expect(normalizeReferralPlate("NKD 5678", false)).toBe("نكد5678");
  });
});
...
describe("bankPlateToArabic", () => {
  ...
  // ملف إحالة حقيقي: «السيارات المتاخره المنطقة الشرقية.xlsx» — عمود
  // Registration Num فيه «7709 ABS» يعني اللوحة «س ب ا 7709».
  //
  // ده تخطيط اللوحة السعودية بالحروف اللاتينية: الرقم قبل الحروف، والحروف
  // اللاتينية **مرآة** لترتيب الحروف العربية (العربي يمين‑لشمال واللاتيني
  // شمال‑ليمين). البرنامج كان بيقراها حرف بحرف كما هي فيطلّع «7709ابس» —
  // الحروف مقلوبة والأرقام في الأول — فمافيش لوحة واحدة كانت بتطابق.
  //
  // القاعدة: لو الأرقام جات **قبل** الحروف اللاتينية → التخطيط لاتيني،
  // فالحروف تتعكس وتترتّب حروف‑ثم‑أرقام. لو الحروف جات الأول (زي NKD 5678)
  // فالترتيب عربي أصلاً ومايتلمسش.
  describe("تخطيط لاتيني (الرقم قبل الحروف) — الحروف مرآة", () => {
    it("«7709 ABS» = س ب ا 7709 مش ابس", () => {
      expect(bankPlateToArabic("7709 ABS")).toBe("سبا7709");
    });

    it("يطابق كل صفوف الملف الحقيقي", () => {
      const rows: Array<[string, string]> = [
        ["7709 ABS", "سبا7709"], ["1714 VVR", "ريي1714"],
        ["3136 UJS", "سحو3136"], ["6534 KBS", "سبك6534"],
        ["9918 JAS", "ساح9918"], ["6210 KHR", "رهك6210"],
        ["8266 VTB", "بطي8266"], ["1839 DEB", "بعد1839"],
        ["8547 SDS", "سدس8547"], ["8939 GER", "رعق8939"],
      ];
      for (const [en, ar] of rows) expect(bankPlateToArabic(en)).toBe(ar);
    });

    it("يشيل الفواصل والشرطات في نفس التخطيط", () => {
      expect(bankPlateToArabic("7709-ABS")).toBe("سبا7709");
      expect(bankPlateToArabic("  9918   JAS  ")).toBe("ساح9918");
    });

    it("لا يعكس لما الحروف تكون الأول (ترتيب عربي)", () => {
      expect(bankPlateToArabic("NKD 5678")).toBe("نكد5678");
      expect(bankPlateToArabic("ABS 7709")).toBe("ابس7709");
    });

    it("حرف واحد أو رقم لوحده ما يتأثرش", () => {
      expect(bankPlateToArabic("7709 A")).toBe("ا7709");
      expect(bankPlateToArabic("7709")).toBe("7709");
      expect(bankPlateToArabic("ABS")).toBe("ابس");
    });
  });
});
