...
## تحديث: إعادة بناء وحدة الفرز + اللصق النصي ✅

- **إصلاح اتجاه النص (RTL)**: تطبيق `direction: rtl` و `unicode-bidi: plaintext`
  بشكل عام على كل حاويات الفرز/التشيك/اللصق (كلاس `.rtl-text` في `globals.css`،
  يشمل العناصر الفرعية كلها) — يمنع انعكاس الحروف العربية المختلطة بأرقام/حروف
  لاتينية القادمة من ملفات Excel المرفوعة.
- **خرائط أعمدة ديناميكية حقيقية**: بعد رفع أي ملف، تظهر أعمدته الفعلية (أيًا
  كانت أسماؤها — بما فيها رقم الشاصي مثلاً) كخيارات تحديد، بدل قائمة ثابتة.
- **لكل نتيجة**: زر نسخ (ينسخ تفصيل الأعمدة المختارة)، زر مشاركة (Web Share API
  لمشاركة نصية مباشرة على واتساب/غيره)، وزر تصدير Excel يضم فقط الأعمدة المختارة.
- **اللصق النصي يعرض المتطابق فقط الآن**: بدل عرض كل اللوحات الملصوقة، يعرض فقط
  ما تم العثور عليه داخل ملف الداتا مع بياناته الكاملة (نفس منطق اختيار الأعمدة).
- **حفظ الملفات محليًا (IndexedDB)**: أي ملف "داتا" أو "إحالة" يُرفع يبقى محفوظًا
  على الجهاز حتى يحذفه العميل يدويًا بأيقونة السلة 🗑️. يدعم .xlsx و .xls و .csv.
- **بدون كلمات مرور إلزامية على الحفظ/المشاركة في الفرز/اللصق** — تبقى حماية
  كلمة المرور الثانوية كما هي فقط في صفحة "التشيك" وتصدير قاعدة البيانات الكاملة.
- **منع تجمّد الواجهة**: التحليل الفعلي للملفات يحدث على الخادم (لا يحظر الواجهة
  أصلًا)، وأضفنا تحميلًا متدرجًا للنتائج (50 صف بكل مرة) بدل رسم آلاف الصفوف دفعة
  واحدة.
